	...
	std::vector<std::wstring>& filenames)
{
	...
	for (UINT i = 0; i < size; ++i)
	{
		...
		HRTex(CreateDDSTextureFromFile(device,
			filenames[i].c_str(),
			&srcTex[i],
			&shaderResourceView));
	...
	HRTex(device->CreateTexture2D(&texArrayDesc, 0, &texArray));
	...
	HRTex(device->CreateShaderResourceView(texArray, &viewDesc, &texArraySRV));
...
ID3D11ShaderResourceView* d3dHelper::CreateRandomTexture1DSRV(ID3D11Device* device)
{
	...
	HRTex(device->CreateTexture1D(&texDesc, &initData, &randomTex));
	...
	HRTex(device->CreateShaderResourceView(randomTex, &viewDesc, &randomTexSRV));
