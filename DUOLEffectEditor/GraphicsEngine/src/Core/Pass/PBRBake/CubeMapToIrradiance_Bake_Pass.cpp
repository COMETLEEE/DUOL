...
MuscleGrapics::CubeMapToIrradiance_Bake_Pass::CubeMapToIrradiance_Bake_Pass() : Pass_Texture(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST)
{
	...
	resoureManager->CompilePixelShader(pipeLineDesc, TEXT("Asset/Particle/Shader/Shader_CubeMapToIrradianceMap.hlsl"), "PSMain");

	InsertShader(pipeLineDesc);

	CreateConstantBuffer(1, sizeof(cbPerObject));
}

void MuscleGrapics::CubeMapToIrradiance_Bake_Pass::SetConstants(
...
void MuscleGrapics::CubeMapToIrradiance_Bake_Pass::Draw(
	std::vector<std::pair<ID3D11ShaderResourceView*, int>>& renderingData)
{
	SetShader();
