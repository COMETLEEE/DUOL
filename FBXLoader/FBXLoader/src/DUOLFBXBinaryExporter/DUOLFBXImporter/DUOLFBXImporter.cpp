...
#pragma region Bounding Box
		...
		{
			dMax.x = max(dMax.x, vertex.position.x);
			dMax.y = max(dMax.y, vertex.position.y);
			dMax.z = max(dMax.z, vertex.position.z);

			dMin.x = min(dMin.x, vertex.position.x);
			dMin.y = min(dMin.y, vertex.position.y);
			dMin.z = min(dMin.z, vertex.position.z);
		}
...
#pragma endregion
	...
	for (int i = 0; i < vertexcount; i++)
	{
		...
		/*if(meshinfo->isSkinned)
			meshinfo->tempVertexList[i].position *= DUOLMath::Vector3(0.01f,0.01f,0.01f);*/
